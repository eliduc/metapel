...
window.MetapelEnv = (function () {
  ...
  return {
    ...
    // правды, остальные устройства сами видят его изменения). Включено в ОБЕИХ
    // средах после обкатки. Безопасность: decideSync подтягивает только когда
    // локально терять нечего (пусто или совпало с последней синхронизацией);
    // несохранённые локальные правки НЕ перезаписываются (конфликт → ручное
    // «Восстановить»). Чтобы временно отключить — поставить здесь false.
    autoSync: true,
